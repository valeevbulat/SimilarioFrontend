function translit(str, typ) {
    let func = (function(typ) {
         ...
         **/
        function prep (a) {
            let write = !a ? function(chr, row) {trantab[row] = chr;regarr.push(row);} :
                function(row, chr) {trantab[row] = chr;regarr.push(row);};
            return function(col, row) {        // создаем таблицу и RegExp
                let chr = col[abs] || col[0];    // Символ
                if (chr) write(chr, row);        // Если символ есть
            }
        }
        let abs = Math.abs(typ);             // Абсолютное значение транслитерации
    ...
    let iso9 = {                           // Объект описания стандарта
        // Имя - кириллица
        //   0 - общие для всех
    ...
    for (let row in iso9) {if (Object.hasOwnProperty.call(iso9, row)) {func[0](iso9[row], row);}}
    /* jshint +W030 */
...
const rus = 'абвгдеёжзийклмнопрстуфхцшщчьыъэюя';
